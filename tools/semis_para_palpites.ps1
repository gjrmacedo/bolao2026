<#
.SYNOPSIS
  Lê os palpites das semis em formato texto (data\palpites\semis\BOLAO2026_NOME.txt)
  e injeta os jogos das semis no JSON correspondente do participante (data\palpites\BOLAO2026_NOME.json).

.DESCRIPTION
  Cada arquivo .txt tem uma linha por jogo, no formato recebido pelo grupo:

      JOGO 101: França 0 X 0 Espanha*
      JOGO 102: Inglaterra* 2 X 2 Argentina

  Regras:
    - O número do JOGO é o próprio id do jogo no data\jogos.json (fase semi).
    ...
    - Os palpites de 1..100 (e quaisquer outros já existentes) são preservados na ordem
      original; os jogos das semis são adicionados/atualizados ao final.

.PARAMETER Path
  Caminho de um .txt específico. Se omitido, processa data\palpites\semis\*.txt.

.PARAMETER TxtDir
  Pasta dos .txt (default: ..\data\palpites\semis relativo ao script).
...
.EXAMPLE
  .\tools\semis_para_palpites.ps1
  .\tools\semis_para_palpites.ps1 -Path .\data\palpites\semis\BOLAO2026_Macedo.txt
#>
[CmdletBinding()]
param(
  [string]$Path,
...
if (-not $TxtDir)    { $TxtDir    = Join-Path $scriptDir '..\data\palpites\semis' }
...
if ($idsEsperados.Count -eq 0) {
  Write-Host "ERRO: Nenhum jogo de semi encontrado em jogos.json com fase='semi'" -ForegroundColor Red
  exit 1
}
...
foreach ($arq in $arquivos) {
  ...
  foreach ($raw in $linhas) {
    ...
    if (-not $jogoSemis.ContainsKey($id)) {
      $erros.Add("JOGO $id não é um jogo das semis.")
      continue
    }
  ...
  Write-Host ("  OK — {0} jogo(s) das semis gravado(s) em {1}" -f $idsEsperados.Count, (Split-Path -Leaf $jsonFile)) -ForegroundColor Green
  $totalOk++
}
