<#
.SYNOPSIS
  Lê os palpites das quartas em formato texto (data\palpites\quartas\BOLAO2026_NOME.txt)
  e injeta os jogos das quartas no JSON correspondente do participante (data\palpites\BOLAO2026_NOME.json).

.DESCRIPTION
  Cada arquivo .txt tem uma linha por jogo, no formato recebido pelo grupo:

      JOGO 97: Paraguai 0 X 2 França
      JOGO 98: Canadá 2 X 2 Marrocos*
      JOGO 100: Suíça* 0 X 0 Colômbia

  Regras:
    - O número do JOGO é o próprio id do jogo no data\jogos.json (fase quartas).
    ...
    - Os palpites de 1..96 (e quaisquer outros já existentes) são preservados na ordem
      original; os jogos das quartas são adicionados/atualizados ao final.

.PARAMETER Path
  Caminho de um .txt específico. Se omitido, processa data\palpites\quartas\*.txt.

.PARAMETER TxtDir
  Pasta dos .txt (default: ..\data\palpites\quartas relativo ao script).
...
.EXAMPLE
  .\tools\quartas_para_palpites.ps1
  .\tools\quartas_para_palpites.ps1 -Path .\data\palpites\quartas\BOLAO2026_Macedo.txt
#>
[CmdletBinding()]
param(
  [string]$Path,
...
if (-not $TxtDir)    { $TxtDir    = Join-Path $scriptDir '..\data\palpites\quartas' }
...
# ---- Jogos de quartas (descobrir ids pela fase no jogos.json) ----
...
foreach ($j in $dados.jogos) {
  if ($j.fase -eq 'quartas') {
    $jogoQuartas[[int]$j.id] = $j
...
if ($idsEsperados.Count -eq 0) {
  Write-Host "ERRO: Nenhum jogo de quartas encontrado em jogos.json com fase='quartas'" -ForegroundColor Red
  exit 1
}
...
foreach ($arq in $arquivos) {
  ...
  foreach ($raw in $linhas) {
    ...
    if (-not $jogoQuartas.ContainsKey($id)) {
      $erros.Add("JOGO $id não é um jogo das quartas.")
      continue
    }
  ...
  # adiciona/atualiza as quartas na ordem dos ids
  ...
  Write-Host ("  OK — {0} jogo(s) das quartas gravado(s) em {1}" -f $idsEsperados.Count, (Split-Path -Leaf $jsonFile)) -ForegroundColor Green
  $totalOk++
}
