<#
.SYNOPSIS
  Lê os palpites das oitavas em formato texto (data\palpites\oitavas\BOLAO2026_NOME.txt)
  e injeta os jogos 89..96 no JSON correspondente do participante (data\palpites\BOLAO2026_NOME.json).

.DESCRIPTION
  Cada arquivo .txt tem uma linha por jogo, no formato recebido pelo grupo:

      JOGO 89: Paraguai 0 X 2 França
      JOGO 90: Canadá 2 X 2 Marrocos*
      JOGO 96: Suíça* 0 X 0 Colômbia

  Regras:
    - O número do JOGO é o próprio id do jogo no data\jogos.json (89..96).
    ...
    - Os palpites de 1..88 (e quaisquer outros já existentes) são preservados na ordem
      original; os jogos 89..96 são adicionados/atualizados ao final.

  Por padrão processa TODOS os .txt da pasta. Use -Path para um arquivo específico.

.PARAMETER Path
  Caminho de um .txt específico. Se omitido, processa data\palpites\oitavas\*.txt.

.PARAMETER TxtDir
  Pasta dos .txt (default: ..\data\palpites\oitavas relativo ao script).
...
.EXAMPLE
  .\tools\oitavas_para_palpites.ps1
  .\tools\oitavas_para_palpites.ps1 -Path .\data\palpites\oitavas\BOLAO2026_Viniciusmartins.txt
#>
[CmdletBinding()]
param(
  [string]$Path,
...
if (-not $TxtDir)    { $TxtDir    = Join-Path $scriptDir '..\data\palpites\oitavas' }
...
foreach ($arq in $arquivos) {
  ...
  foreach ($raw in $linhas) {
    ...
    if (-not $jogo16.ContainsKey($id)) {
      $erros.Add("JOGO $id não é um jogo das oitavas (esperado 89..96).")
      continue
    }
  ...
  # adiciona/atualiza 89..96 na ordem dos ids
  ...
  Write-Host ("  OK — 8 jogos (89..96) gravados em {0}" -f (Split-Path -Leaf $jsonFile)) -ForegroundColor Green
  $totalOk++
}
